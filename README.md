# Data Scientist | Environmental & Earth Systems

I apply data science to real-world sustainability and finance problems. My work spans climate risk tools for SMEs, neobank research for freelancers, and wet-lab → data pipelines in carbon capture. I love turning messy data into decisions.

### Education
**Minerva University** — B.Sc. Environmental & Earth Systems, **Minor: Data Science & Statistics**  
*Global rotation • Aug 2023–May 2027*  
Relevant coursework: Data Structures & Algorithms, Linear Algebra, Probability & Statistics, Environmental Modeling, Chemical Structures & Reactivity, Calculus (Single & Multivariable), Physics of Life, Empirical/Formal Analyses.


### Experience
**STEM Instructor (Robotics, AI, Python) — Lavner Education**  
*San Diego, CA • Jun–Aug 2025*  
- Designed & taught hands-on courses (Python, Scratch, Arduino, LEGO Mindstorms) for ages 8–14.  
- Built activity rubrics and mini-projects that boosted retention and engagement.

**Research Trainee — KIST A-Labs (CCUS Group)**  
*Seoul, South Korea • Jan–Apr 2025*  
- Ran slaking/causticizing experiments (CaO → Ca(OH)₂ → CaCO₃); logged and analyzed variables in Python/Excel.  
- Proposed process-control checkpoints to stabilize large-scale solution regeneration.

**Educational Consultant — SmarTestPrep (EdTech)**  
*Remote (CIS) • Jun 2024–Jan 2025*  
- Mentored 20+ students on essays/portfolios; tracked outcomes with scoring rubrics (10+ global admits).  
- Built advising frameworks informed by past cohort data.

**Payment Solutions Intern — Mastercard (Kazakhstan)**  
*Almaty, KZ • Jun–Aug 2024*  
- Quant + qual research on IT freelancers’ payments; authored a 30-page neobank roadmap for the gig economy.  
- Contributed market sizing, flows, and MVP features bridging card rails and local banks.

**Climate Adaptation Fellow (Co-curricular) — Gensler**  
*San Francisco, CA • Oct 2023–Apr 2024*  
- Co-developed a public website that centralizes grants & climate-resilience resources for 100+ small businesses.  
- Led user interviews in climate-vulnerable neighborhoods and synthesized findings into product specs.


### 📌 Selected Projects
- **Climate-Action Toolkit** — Resource hub for SME climate grants & resilience tools  
  👉 [climate-action-toolkit.vercel.app](https://climate-action-toolkit.vercel.app/)
- **Tellus Mater** — Minerva’s sustainability newsletter (Editor; 400+ readership)  
  👉 [tellusmater.substack.com](https://tellusmater.substack.com/)
- **DermaMarker (Tumaris Tech)** — Contributed to early MedTech concept; team won “Deep Diving” prize  
  👉 media: [IT Park news article](https://it-park.uz/en/itpark/news/female-startup-a-project-that-helps-identify-serious-disease)

---
### 📫 Contact
- Email: [aru@uni.minerva.edu][email]
- LinkedIn: [Aru on LinkedIn][linkedin]
- Substack: [Tellus Mater][substack]

[email]: mailto:aru@uni.minerva.edu
[linkedin]: https://linkedin.com/in/arukudassova
[substack]: https://tellusmater.substack.com
---
